...
function Ensure-StaMode {
    ...
    if ($state -eq [Threading.ApartmentState]::STA) { return }

    # Prevent relaunch loops in restricted WinPE shells.
    if ($env:WINPE_SS_STA_RELAUNCHED -eq '1') { return }

    $psExe = Join-Path $PSHOME 'powershell.exe'
    if (-not (Test-Path $psExe)) { return }
    ...
    if ($full) { $argList.Add('-full') }
    if ($region) { $argList.Add('-region') }
    if ($window) { $argList.Add('-window') }
    if ($PSBoundParameters.ContainsKey('delay')) {
        $argList.Add('-delay')
        $argList.Add([string]$delay)
    }
    ...
    try {
        ...
        $psi.EnvironmentVariables['WINPE_SS_STA_RELAUNCHED'] = '1'
        $process = [System.Diagnostics.Process]::Start($psi)
        if ($process) { exit }
    } catch {
        # Continue in current process if STA relaunch is unavailable.
    }
}
...
$Script:LastCapturedBitmap = $null
$Script:EditorPictureBox = $null
$Script:EditorBitmap = $null
$Script:HistoryListView = $null
$Script:HistoryImageList = $null
$Script:EditorState = @{
    DrawColor = [System.Drawing.Color]::Red
    Drawing   = $false
    LastPt    = [System.Drawing.Point]::Empty
}
$Script:Ctrl = @{}
$Script:Recording = $false
$Script:RecTimer = $null
$Script:RecFrames = New-Object System.Collections.ArrayList
$Script:RecStartTime = $null
$Script:RecFps = 10
...
$Script:RecFps = [int]$Script:Settings.RecFps
...
function Invoke-RefreshWindowList {
    $lw = $Script:Ctrl.listWindows
    if ($null -eq $lw) { return }
    $lw.Items.Clear()
    foreach ($w in (Get-WindowList)) { [void]$lw.Items.Add($w) }
    if ($lw.Items.Count -gt 0) {
        $lw.SelectedIndex = 0
        $Script:LastSelectedWindow = $lw.SelectedItem
    }
}

function Invoke-LoadSettingsToUi {
    $c = $Script:Ctrl
    if (-not $c.txtFolder) { return }
    $c.txtFolder.Text = [string]$Script:Settings.AutoSaveFolder
    $c.cmbFormat.SelectedItem = [string]$Script:Settings.DefaultFormat
    if (-not $c.cmbFormat.SelectedItem) { $c.cmbFormat.SelectedItem = 'PNG' }
    $c.chkClipboard.Checked = [bool]$Script:Settings.CopyToClipboard
    $c.chkSilent.Checked = [bool]$Script:Settings.SilentMode
    $c.chkQuickCopy.Checked = [bool]$Script:Settings.QuickCopyOnly
    $c.numDelay.Value = [decimal][Math]::Max(0, [Math]::Min(60, [int]$Script:Settings.DelaySeconds))
    $c.txtTemplate.Text = [string]$Script:Settings.FileNameTemplate
    $c.numHist.Value = [decimal][Math]::Max(1, [Math]::Min(200, [int]$Script:Settings.HistoryLimit))
    if ($c.numRecFps) { $c.numRecFps.Value = [decimal][Math]::Max(1, [Math]::Min(30, [int]$Script:Settings.RecFps)) }

    $c.cmbMonitor.Items.Clear()
    $screens = [System.Windows.Forms.Screen]::AllScreens
    for ($i = 0; $i -lt $screens.Count; $i++) {
        $b = $screens[$i].Bounds
        $primary = if ($screens[$i].Primary) { ' (Primary)' } else { '' }
        [void]$c.cmbMonitor.Items.Add("#${i}: $($b.Width)x$($b.Height) at [$($b.X),$($b.Y)]$primary")
    }
    if ($c.cmbMonitor.Items.Count -eq 0) { [void]$c.cmbMonitor.Items.Add('#0: Primary') }
    $idx = [Math]::Max(0, [Math]::Min($c.cmbMonitor.Items.Count - 1, [int]$Script:Settings.MonitorIndex))
    $c.cmbMonitor.SelectedIndex = $idx
}

function Invoke-SaveUiToSettings {
    $c = $Script:Ctrl
    if (-not $c.txtFolder) { return }
    $Script:Settings.AutoSaveFolder = $c.txtFolder.Text.Trim()
    $Script:Settings.DefaultFormat = [string]$c.cmbFormat.SelectedItem
    $Script:Settings.CopyToClipboard = $c.chkClipboard.Checked
    $Script:Settings.SilentMode = $c.chkSilent.Checked
    $Script:Settings.QuickCopyOnly = $c.chkQuickCopy.Checked
    $Script:Settings.DelaySeconds = [int]$c.numDelay.Value
    $Script:Settings.FileNameTemplate = $c.txtTemplate.Text.Trim()
    $Script:Settings.HistoryLimit = [int]$c.numHist.Value
    $Script:Settings.MonitorIndex = [int]$c.cmbMonitor.SelectedIndex
    if ($c.numRecFps) {
        $Script:Settings.RecFps = [int]$c.numRecFps.Value
        $Script:RecFps = [int]$c.numRecFps.Value
    }
...
function Set-LastCapturedBitmap([System.Drawing.Bitmap]$bitmap) {
    if ($null -eq $bitmap) { return }
    if ($Script:LastCapturedBitmap) { $Script:LastCapturedBitmap.Dispose() }
    $Script:LastCapturedBitmap = $bitmap.Clone([System.Drawing.Rectangle]::FromLTRB(0, 0, $bitmap.Width, $bitmap.Height), $bitmap.PixelFormat)
}

function Update-EditorPreview {
    if ($null -eq $Script:EditorPictureBox) { return }
    if ($Script:EditorBitmap) {
        if (-not [Object]::ReferenceEquals($Script:EditorPictureBox.Image, $Script:EditorBitmap)) {
            $Script:EditorPictureBox.Image = $Script:EditorBitmap
        }
        $Script:EditorPictureBox.Width = $Script:EditorBitmap.Width
        $Script:EditorPictureBox.Height = $Script:EditorBitmap.Height
    }
}

function Load-LastIntoEditor {
    if ($null -eq $Script:LastCapturedBitmap) {
        Show-Error 'No screenshot captured yet.'
        return
    }
    if ($Script:EditorBitmap) { $Script:EditorBitmap.Dispose() }
    $Script:EditorBitmap = $Script:LastCapturedBitmap.Clone([System.Drawing.Rectangle]::FromLTRB(0, 0, $Script:LastCapturedBitmap.Width, $Script:LastCapturedBitmap.Height), $Script:LastCapturedBitmap.PixelFormat)
...
function Update-RecordingLabel {
    if (-not $Script:Ctrl.lblRecStatus) { return }
    if ($Script:Recording) {
        $elapsed = [datetime]::Now - $Script:RecStartTime
        $Script:Ctrl.lblRecStatus.Text = ('REC {0} - {1} frames' -f $elapsed.ToString('mm\:ss'), $Script:RecFrames.Count)
        $Script:Ctrl.lblRecStatus.ForeColor = [System.Drawing.Color]::Red
        $Script:Ctrl.btnRecord.Text = 'Stop Recording (F4)'
    } else {
        $Script:Ctrl.lblRecStatus.Text = 'Not recording'
        $Script:Ctrl.lblRecStatus.ForeColor = [System.Drawing.Color]::Gray
        $Script:Ctrl.btnRecord.Text = 'Start Recording (F4)'
    }
}

function Start-Recording {
    if ($Script:Recording) { return }
    [void]$Script:RecFrames.Clear()
    $Script:RecStartTime = [datetime]::Now
    $Script:Recording = $true
    Update-RecordingLabel

    $Script:RecTimer = New-Object System.Windows.Forms.Timer
    $Script:RecTimer.Interval = [int](1000 / [Math]::Max(1, $Script:RecFps))
    $Script:RecTimer.Add_Tick({
        if (-not $Script:Recording) { return }
        try {
            $bmp = Capture-Bounds (Get-SelectedMonitor).Bounds
            if ($bmp) { [void]$Script:RecFrames.Add($bmp) }
        } catch {}
        Update-RecordingLabel
    })
...
function Stop-Recording {
    ...
    Update-RecordingLabel
    if ($Script:RecFrames.Count -eq 0) { Show-Info 'No frames captured.'; return }

    $sfd = New-Object System.Windows.Forms.SaveFileDialog
    $sfd.Title = ('Save Recording ({0} frames)' -f $Script:RecFrames.Count)
    $sfd.Filter = 'MP4 video (*.mp4)|*.mp4|ZIP of PNG frames (*.zip)|*.zip'
    $sfd.FileName = ('recording_{0}' -f (Get-Date -Format 'yyyyMMdd_HHmmss'))
    if ($sfd.ShowDialog() -ne [System.Windows.Forms.DialogResult]::OK) {
        foreach ($f in $Script:RecFrames) { $f.Dispose() }
        [void]$Script:RecFrames.Clear()
        return
    }

    Add-Type -AssemblyName System.IO.Compression.FileSystem
    $tmp = Join-Path ([IO.Path]::GetTempPath()) ('rec_' + [guid]::NewGuid().ToString('N'))
    New-Item -Path $tmp -ItemType Directory -Force | Out-Null
    try {
        for ($i = 0; $i -lt $Script:RecFrames.Count; $i++) {
            $Script:RecFrames[$i].Save((Join-Path $tmp ('frame_{0:D6}.png' -f $i)), [System.Drawing.Imaging.ImageFormat]::Png)
        }
        if (Test-Path $sfd.FileName) { Remove-Item $sfd.FileName -Force }
        if ([IO.Path]::GetExtension($sfd.FileName).ToLowerInvariant() -eq '.mp4') {
            Save-RecordingAsMp4 -framesDir $tmp -outPath $sfd.FileName -fps $Script:RecFps
        } else {
            [System.IO.Compression.ZipFile]::CreateFromDirectory($tmp, $sfd.FileName)
        }
        Show-Info ("Saved: {0}" -f $sfd.FileName)
    } catch {
        Show-Error ("Recording save failed: {0}" -f $_.Exception.Message)
    } finally {
...
function Get-FfmpegPath {
    $candidates = @(
        ...
        (Join-Path $env:SystemRoot 'System32\\ffmpeg.exe'),
        'ffmpeg.exe'
    )
...
function Save-RecordingAsMp4 {
    ...
    if (-not $ffmpeg) {
        throw 'ffmpeg.exe not found. Put ffmpeg.exe рядом со скриптом или в PATH.'
    }

    $inputPattern = Join-Path $framesDir 'frame_%06d.png'
    $args = @(
        '-y',
    ...
    if ($proc.ExitCode -ne 0 -or -not (Test-Path $outPath)) {
        throw ("ffmpeg failed with exit code {0}" -f $proc.ExitCode)
    }
}

function Toggle-Recording {
    if ($Script:Recording) { Stop-Recording } else { Start-Recording }
}
...
function New-MainForm {
    ...
    $tabRecord.Text = 'Record Screen'
    $btnRecord = New-Object System.Windows.Forms.Button
    $btnRecord.Text = 'Start Recording (F4)'
    $btnRecord.Location = New-Object System.Drawing.Point 20, 20
    $btnRecord.Width = 220
    $lblRecStatus = New-Object System.Windows.Forms.Label
    $lblRecStatus.Text = 'Not recording'
    $lblRecStatus.AutoSize = $true
    $lblRecStatus.Location = New-Object System.Drawing.Point 20, 70
    $lblRecStatus.ForeColor = [System.Drawing.Color]::Gray
    $lblRecFps = New-Object System.Windows.Forms.Label
    $lblRecFps.Text = 'FPS:'
    $lblRecFps.AutoSize = $true
    $lblRecFps.Location = New-Object System.Drawing.Point 20, 105
    $numRecFps = New-Object System.Windows.Forms.NumericUpDown
    $numRecFps.Location = New-Object System.Drawing.Point 60, 103
    $numRecFps.Width = 70
    $numRecFps.Minimum = 1
    $numRecFps.Maximum = 30
    $numRecFps.Value = [decimal]$Script:RecFps
    $tabRecord.Controls.AddRange(@($btnRecord, $lblRecStatus, $lblRecFps, $numRecFps))
    ...
    $editorTop = New-Object System.Windows.Forms.Panel
    $editorTop.Dock = [System.Windows.Forms.DockStyle]::Top
    $editorTop.Height = 70

    $btnLoadLast = New-Object System.Windows.Forms.Button
    $btnLoadLast.Text = 'Load Last Screenshot'
    $btnLoadLast.Location = New-Object System.Drawing.Point 10, 20
    $btnLoadLast.Width = 160

    $btnDrawMode = New-Object System.Windows.Forms.CheckBox
    $btnDrawMode.Text = 'Draw'
    $btnDrawMode.Appearance = 'Button'
    $btnDrawMode.Location = New-Object System.Drawing.Point 180, 20
    $btnDrawMode.Width = 70
    $btnDrawMode.TextAlign = 'MiddleCenter'

    $btnTextMode = New-Object System.Windows.Forms.CheckBox
    $btnTextMode.Text = 'Text'
    $btnTextMode.Appearance = 'Button'
    $btnTextMode.Location = New-Object System.Drawing.Point 255, 20
    $btnTextMode.Width = 70
    $btnTextMode.TextAlign = 'MiddleCenter'

    $btnColor = New-Object System.Windows.Forms.Button
    $btnColor.Text = 'Color'
    $btnColor.Location = New-Object System.Drawing.Point 330, 20
    $btnColor.Width = 70
    $btnColor.BackColor = $Script:EditorState.DrawColor

    $lblPen = New-Object System.Windows.Forms.Label
    $lblPen.Text = 'Size:'
    $lblPen.Location = New-Object System.Drawing.Point 410, 25
    $lblPen.AutoSize = $true

    $numPen = New-Object System.Windows.Forms.NumericUpDown
    $numPen.Location = New-Object System.Drawing.Point 450, 22
    $numPen.Minimum = 1
    $numPen.Maximum = 30
    $numPen.Value = 2
    $numPen.Width = 60

    $txtDrawText = New-Object System.Windows.Forms.TextBox
    $txtDrawText.Location = New-Object System.Drawing.Point 520, 22
    $txtDrawText.Width = 170
    $txtDrawText.Text = 'Sample text'

    $btnSaveEdited = New-Object System.Windows.Forms.Button
    $btnSaveEdited.Text = 'Save Edited'
    $btnSaveEdited.Location = New-Object System.Drawing.Point 700, 20
    $btnSaveEdited.Width = 90

    $btnCopyEdited = New-Object System.Windows.Forms.Button
    $btnCopyEdited.Text = 'Copy'
    $btnCopyEdited.Location = New-Object System.Drawing.Point 795, 20
    $btnCopyEdited.Width = 60

    $editorTop.Controls.AddRange(@(
        $btnLoadLast, $btnDrawMode, $btnTextMode, $btnColor, $lblPen, $numPen, $txtDrawText, $btnSaveEdited, $btnCopyEdited
    ))
    ...
    $editorContainer.BackColor = [System.Drawing.Color]::FromArgb(45,45,48)

    $Script:EditorPictureBox = New-Object System.Windows.Forms.PictureBox
    $Script:EditorPictureBox.Location = New-Object System.Drawing.Point 0, 0
    $Script:EditorPictureBox.SizeMode = [System.Windows.Forms.PictureBoxSizeMode]::Normal
    $editorContainer.Controls.Add($Script:EditorPictureBox)

    $tabEditor.Controls.Add($editorContainer)
    $tabEditor.Controls.Add($editorTop)
    ...
    $tabs.TabPages.AddRange(@($tabFull, $tabRegion, $tabWindow, $tabRecord, $tabSettings, $tabEditor, $tabHistory))
    ...
    $Script:Ctrl = @{
        ...
        chkSilent = $chkSilent
        chkQuickCopy = $chkQuickCopy
        numDelay = $numDelay
        txtTemplate = $txtTemplate
        numHist = $numHist
        cmbMonitor = $cmbMonitor
        listWindows = $listWindows
        btnDrawMode = $btnDrawMode
        btnTextMode = $btnTextMode
        numPen = $numPen
        txtDrawText = $txtDrawText
        tabs = $tabs
        tabEditor = $tabEditor
        btnColor = $btnColor
        btnRecord = $btnRecord
        lblRecStatus = $lblRecStatus
        numRecFps = $numRecFps
    }
    ...
    $btnSaveSettings.Add_Click({
        Invoke-SaveUiToSettings
        Show-Info 'Settings saved.'
        Update-HistoryUi
        Update-RecordingLabel
    })

    $btnRefreshWin.Add_Click({ Invoke-RefreshWindowList })
    ...
    $btnFull.Add_Click({ Invoke-SaveUiToSettings; Run-FullCapture $form })
    $btnRegion.Add_Click({ Invoke-SaveUiToSettings; Run-RegionCapture $form })
    $btnWindow.Add_Click({
        Invoke-SaveUiToSettings
    ...
    $btnRecord.Add_Click({ Invoke-SaveUiToSettings; Toggle-Recording })

    $btnColor.Add_Click({
        $cd = New-Object System.Windows.Forms.ColorDialog
        $cd.Color = $Script:EditorState.DrawColor
        if ($cd.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
            $Script:EditorState.DrawColor = $cd.Color
            $btnColor.BackColor = $Script:EditorState.DrawColor
        }
    })

    $btnDrawMode.Add_CheckedChanged({
        if ($btnDrawMode.Checked) { $btnTextMode.Checked = $false }
    })
    $btnTextMode.Add_CheckedChanged({
        if ($btnTextMode.Checked) { $btnDrawMode.Checked = $false }
    })

    $btnLoadLast.Add_Click({ Load-LastIntoEditor })

    $Script:EditorPictureBox.Add_MouseDown({
        if (-not $Script:EditorBitmap) { return }
        if ($_.Button -ne [System.Windows.Forms.MouseButtons]::Left) { return }
        if ($btnDrawMode.Checked) {
            $Script:EditorState.Drawing = $true
            $Script:EditorState.LastPt = $_.Location
        } elseif ($btnTextMode.Checked) {
            $g = [System.Drawing.Graphics]::FromImage($Script:EditorBitmap)
            try {
                $font = New-Object System.Drawing.Font('Arial', 18, [System.Drawing.FontStyle]::Bold)
                $brush = New-Object System.Drawing.SolidBrush($Script:EditorState.DrawColor)
                try {
                    $text = $txtDrawText.Text
                    if ([string]::IsNullOrWhiteSpace($text)) { $text = 'Text' }
                    $g.DrawString($text, $font, $brush, $_.X, $_.Y)
                } finally {
                    $brush.Dispose()
                    $font.Dispose()
                }
    ...
    $Script:EditorPictureBox.Add_MouseMove({
        if (-not $Script:EditorState.Drawing -or -not $Script:EditorBitmap) { return }
        $g = [System.Drawing.Graphics]::FromImage($Script:EditorBitmap)
        $pen = $null
        try {
            $pen = New-Object System.Drawing.Pen($Script:EditorState.DrawColor, [float]$numPen.Value)
            $pen.StartCap = [System.Drawing.Drawing2D.LineCap]::Round
            $pen.EndCap = [System.Drawing.Drawing2D.LineCap]::Round
            $g.SmoothingMode = [System.Drawing.Drawing2D.SmoothingMode]::AntiAlias
            $g.DrawLine($pen, $Script:EditorState.LastPt, $_.Location)
            $Script:EditorState.LastPt = $_.Location
        } finally {
            if ($pen) { $pen.Dispose() }
            $g.Dispose()
    ...
    $Script:EditorPictureBox.Add_MouseUp({
        $Script:EditorState.Drawing = $false
    })

    $btnSaveEdited.Add_Click({
        if (-not $Script:EditorBitmap) {
            Show-Error 'Load a screenshot into Editor first.'
            return
        }
        $tmp = $Script:EditorBitmap.Clone([System.Drawing.Rectangle]::FromLTRB(0, 0, $Script:EditorBitmap.Width, $Script:EditorBitmap.Height), $Script:EditorBitmap.PixelFormat)
        try { [void](Save-OrCopyBitmap $tmp) } finally { $tmp.Dispose() }
    })

    $btnCopyEdited.Add_Click({
        if (-not $Script:EditorBitmap) {
            Show-Error 'Load a screenshot into Editor first.'
            return
        }
        try {
            [System.Windows.Forms.Clipboard]::SetImage($Script:EditorBitmap)
            Show-Info 'Edited image copied to clipboard.'
        } catch {
            Show-Error 'Clipboard copy failed.'
        }
    })

    $tabs.Add_SelectedIndexChanged({
        if ($tabs.SelectedTab -eq $tabEditor -and -not $Script:EditorBitmap -and $Script:LastCapturedBitmap) {
            Load-LastIntoEditor
        }
    })
    ...
    $form.Add_KeyDown({
        switch ($_.KeyCode) {
            ([System.Windows.Forms.Keys]::F1) { Invoke-SaveUiToSettings; Run-FullCapture $form; $_.Handled = $true }
            ([System.Windows.Forms.Keys]::F2) { Invoke-SaveUiToSettings; Run-RegionCapture $form; $_.Handled = $true }
            ...
            ([System.Windows.Forms.Keys]::F4) { Invoke-SaveUiToSettings; Toggle-Recording; $_.Handled = $true }
        }
    })

    $form.Add_Shown({
        Invoke-LoadSettingsToUi
        Invoke-RefreshWindowList
        Update-HistoryUi
        Update-RecordingLabel
...
try {
    ...
    $fatal = "Fatal error: $($_.Exception.Message)"
    try {
