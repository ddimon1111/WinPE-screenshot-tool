...
SCAPY_IMPORT_WARNING = None
try:
    with redirect_stderr(io.StringIO()) as _buf:
        from scapy.all import sniff, conf  # type: ignore
    SCAPY_IMPORT_WARNING = _buf.getvalue().strip() or None
except Exception as _scapy_err:
    sniff = None
    conf = None
    SCAPY_IMPORT_WARNING = str(_scapy_err)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.build_ui()
        self.bind_hotkeys()
        if SCAPY_IMPORT_WARNING:
            self.log(f"Scapy предупреждение/ошибка: {SCAPY_IMPORT_WARNING}", "unknown")
    ...
    def build_ui(self):
        ...
        self.tab_antirat = ttk.Frame(self.notebook)
        ...
        self.notebook.add(self.tab_antirat, text="АнтИRaT")
    ...
    def build_system_tab(self):
        ...
        ttk.Button(top, text="Обновить автозагрузку", command=self.show_all_autoruns).pack(side="left", padx=3)
        ttk.Button(top, text="Скан блокировщиков", command=self.scan_system_blockers).pack(side="left", padx=3)
        ttk.Button(top, text="Разблокировать", command=self.unlock_system_tools).pack(side="left", padx=3)

        ttk.Label(self.tab_system, text="Раздел: Автозагрузка").pack(anchor="w", padx=10)
        self.autorun_nb = ttk.Notebook(self.tab_system)
        self.autorun_nb.pack(fill="both", expand=True, padx=8, pady=6)

        self.autorun_tabs = {}
        for name in ["Run", "RunOnce", "Winlogon", "Task Scheduler (Tskshc)", "Other"]:
            frame = ttk.Frame(self.autorun_nb)
            text = ScrolledText(frame, font=("Consolas", 10))
            text.pack(fill="both", expand=True, padx=5, pady=5)
            self.autorun_nb.add(frame, text=name)
            self.autorun_tabs[name] = text

    def show_autoruns_registry(self):
        if not winreg:
            return self.write_autorun_tab("Run", "Реестр недоступен (не Windows)")
        paths = [
                ...
                with winreg.OpenKey(hive, key_path) as key:
                    ...
                    while True:
                        try:
                            ...
                            self.write_autorun_tab("Run", f"{name}\\Run: {n} = {v}")
                            i += 1
                        except OSError:
                            break
            except Exception as e:
                self.write_autorun_tab("Run", f"{name}\\Run error: {e}")

    def show_autoruns_scheduler(self):
        try:
            p = subprocess.run("schtasks /query /fo LIST /v", shell=True, capture_output=True, text=True)
            out = ((p.stdout or "") + (p.stderr or "")).splitlines()
            for ln in out[:300]:
                self.write_autorun_tab("Task Scheduler (Tskshc)", ln)
        except Exception as e:
            self.write_autorun_tab("Task Scheduler (Tskshc)", f"error: {e}")

    def show_all_autoruns(self):
        for widget in self.autorun_tabs.values():
            widget.delete("1.0", "end")
        self.show_autoruns_registry()
        self.show_autoruns_runonce()
        self.show_autoruns_winlogon()
        self.show_autoruns_scheduler()
        self.show_autoruns_other()
        self.log("Автозагрузка обновлена по всем подвкладкам", "info")

    def scan_system_blockers(self):
        if not winreg:
            return self.log("Скан блокировщиков: реестр недоступен", "unknown")
        checks = [
            ...
            (r"Software\Microsoft\Windows\CurrentVersion\Policies\System", "DisableMSConfig"),
        ]
        found = []
        for root, root_name in [(winreg.HKEY_CURRENT_USER, "HKCU"), (winreg.HKEY_LOCAL_MACHINE, "HKLM")]:
            for kp, val_name in checks:
                try:
                    with winreg.OpenKey(root, kp) as key:
                        val, _ = winreg.QueryValueEx(key, val_name)
                        if str(val) in {"1", "True", "true"}:
                            found.append((root, root_name, kp, val_name))
                            self.log(f"БЛОКИРОВЩИК: {root_name}\\{kp} -> {val_name}={val}", "malicious")
                except Exception:
                    continue
        if not found:
            self.log("Блокировщики не найдены", "clean")
        self._last_blockers = found

    def unlock_system_tools(self):
        if not winreg:
            return self.log("Разблокировка: реестр недоступен", "unknown")
        if not hasattr(self, "_last_blockers"):
            self.scan_system_blockers()
        removed = 0
        for root, root_name, kp, val_name in getattr(self, "_last_blockers", []):
            try:
                with winreg.OpenKey(root, kp, 0, winreg.KEY_SET_VALUE) as key:
                    winreg.DeleteValue(key, val_name)
                removed += 1
                self.log(f"Разблокировано: {root_name}\\{kp} -> {val_name}", "clean")
            except Exception as e:
                self.log(f"Не удалось удалить {root_name}\\{kp} -> {val_name}: {e}", "error")
        if removed == 0:
            self.log("Разблокировка: удалять нечего", "unknown")

    def write_autorun_tab(self, tab_name, line):
        w = self.autorun_tabs.get(tab_name)
        if w:
            w.insert("end", line + "\n")
            w.see("end")
        self.log(f"[{tab_name}] {line}", "unknown")

    def show_autoruns_runonce(self):
        if not winreg:
            return self.write_autorun_tab("RunOnce", "Реестр недоступен (не Windows)")
        paths = [
            ...
            try:
                with winreg.OpenKey(hive, key_path) as key:
                    ...
                    while True:
                        try:
                            n, v, _ = winreg.EnumValue(key, i)
                            self.write_autorun_tab("RunOnce", f"{name}\\RunOnce: {n} = {v}")
                            i += 1
                        except OSError:
                            break
            except Exception:
                self.write_autorun_tab("RunOnce", f"{name}\\RunOnce: нет данных/доступа")

    def show_autoruns_winlogon(self):
        if not winreg:
            return self.write_autorun_tab("Winlogon", "Реестр недоступен (не Windows)")
        key_path = r"Software\Microsoft\Windows NT\CurrentVersion\Winlogon"
        for hive, name in [(winreg.HKEY_LOCAL_MACHINE, "HKLM"), (winreg.HKEY_CURRENT_USER, "HKCU")]:
            try:
                with winreg.OpenKey(hive, key_path) as key:
                    for value_name in ["Shell", "Userinit", "Notify"]:
                        try:
                            value, _ = winreg.QueryValueEx(key, value_name)
                            self.write_autorun_tab("Winlogon", f"{name}\\{key_path}: {value_name} = {value}")
                        except Exception:
                            continue
            except Exception:
                self.write_autorun_tab("Winlogon", f"{name}\\{key_path}: нет данных/доступа")

    def show_autoruns_other(self):
        startup_dirs = [
        ...
        self.write_autorun_tab("Other", "Startup folders:")
        for d in startup_dirs:
            if d.exists():
                for item in d.iterdir():
                    self.write_autorun_tab("Other", f"{d}: {item.name}")
            else:
                self.write_autorun_tab("Other", f"{d}: not found")

        for cmd, label in [
            ("sc query type= service state= all", "Services"),
            ("driverquery /v", "Drivers"),
        ]:
            try:
                p = subprocess.run(cmd, shell=True, capture_output=True, text=True)
                out = ((p.stdout or "") + (p.stderr or "")).splitlines()[:120]
                self.write_autorun_tab("Other", f"{label} snapshot:")
                for ln in out:
                    self.write_autorun_tab("Other", ln)
            except Exception as e:
                self.write_autorun_tab("Other", f"{label} error: {e}")

        if winreg:
            self.write_autorun_tab("Other", "Shell-extensions:")
            keys = [
                    ...
                    try:
                        with winreg.OpenKey(root, kp) as key:
                            ...
                            while True:
                                try:
                                    n, v, _ = winreg.EnumValue(key, i)
                                    self.write_autorun_tab("Other", f"{root_name}\\{kp}: {n} = {v}")
                                    i += 1
                                except OSError:
                                    break
                    except Exception:
    ...
    def start_sniffer(self):
        if sniff is None:
            self.log("Scapy/libpcap недоступны, включён fallback-sniffer через net_connections", "unknown")
            return self.start_sniffer_fallback()
        if self.sniffer_running:
            return
        self.sniffer_running = True
        if conf is not None and not getattr(conf, "use_pcap", False):
            self.log("Scapy работает без libpcap: используем fallback-sniffer", "unknown")
            self.sniffer_running = False
            return self.start_sniffer_fallback()
    ...
    def start_sniffer_fallback(self):
        if self.sniffer_running:
            return
        self.sniffer_running = True

        def run():
            self.log("Fallback-sniffer запущен (psutil.net_connections)", "info")
            prev = set()
            while self.sniffer_running:
                cur = set()
                for c in psutil.net_connections(kind="inet"):
                    l = f"{c.laddr.ip}:{c.laddr.port}" if c.laddr else "-"
                    r = f"{c.raddr.ip}:{c.raddr.port}" if c.raddr else "-"
                    cur.add((c.pid or 0, l, r, c.status))
                for item in cur - prev:
                    pid, l, r, st = item
                    self.log(f"Fallback-sniffer NEW: PID={pid} {l} -> {r} [{st}]", "unknown")
                prev = cur
                time.sleep(2)
            self.log("Fallback-sniffer остановлен", "info")
    ...
    def apply_safe_mode(self, mode):
        ...
        if mode == "minimal":
            for t in [self.tab_internet, self.tab_usb, self.tab_boot, self.tab_scripts, self.tab_antirat]:
    ...
    # ---------- AntiRAT ----------
    def build_antirat_tab(self):
        top = ttk.Frame(self.tab_antirat)
        top.pack(fill="x", padx=8, pady=8)
        ttk.Button(top, text="Сканировать RAT-признаки", command=self.scan_antirat).pack(side="left", padx=3)
        ttk.Button(top, text="Изолировать", command=self.isolate_antirat_target).pack(side="left", padx=3)
        ttk.Label(
            self.tab_antirat,
            text="Проверки: подозрительные соединения, необычная автозагрузка, скрытые службы",
        ).pack(anchor="w", padx=10, pady=4)

    def scan_antirat(self):
        threading.Thread(target=self.scan_antirat_thread, daemon=True).start()

    def scan_antirat_thread(self):
        suspicious_ports = {4444, 5555, 6666, 7777, 1337, 9001}
        suspicious_hits = []
        self.log("АнтИRaT: старт сканирования", "info")
        ...
        for c in psutil.net_connections(kind="inet"):
            ...
            pid = c.pid or 0
            port = c.raddr.port
            if port in suspicious_ports or c.status in {"SYN_SENT", "CLOSE_WAIT"}:
                try:
                    pname = psutil.Process(pid).name() if pid else "-"
                except Exception:
                    pname = "?"
                msg = f"Подозрительное соединение: PID={pid} {pname} -> {c.raddr.ip}:{port} [{c.status}]"
                suspicious_hits.append(("process", pid, pname))
                self.log(msg, "suspicious")

        autorun_markers = ["temp", "appdata", "powershell -w hidden", "cmd /c", "vbs", ".js"]
        try:
            for p in psutil.process_iter(["pid", "name", "exe", "cmdline"]):
                txt = " ".join((p.info.get("cmdline") or []))
                txt += " " + str(p.info.get("exe") or "")
                low = txt.lower()
                if any(m in low for m in autorun_markers):
                    self.log(f"Необычный процесс/автозапуск: PID={p.pid} {p.info.get('name','?')} | {txt[:180]}", "suspicious")
                    suspicious_hits.append(("process", p.pid, p.info.get("name", "?")))
        except Exception as e:
            self.log(f"АнтИRaT: ошибка проверки процессов: {e}", "error")

        try:
            p = subprocess.run("sc query type= service state= all", shell=True, capture_output=True, text=True)
            text = (p.stdout or "") + (p.stderr or "")
            for line in text.splitlines():
                if re.search(r"DISPLAY_NAME|SERVICE_NAME", line, flags=re.I):
                    low = line.lower()
                    if any(x in low for x in ["rat", "remote", "hidden", "backdoor"]):
                        self.log(f"Подозрительная служба: {line.strip()}", "suspicious")
        except Exception as e:
            self.log(f"АнтИRaT: ошибка проверки служб: {e}", "error")

        self._antirat_hits = suspicious_hits
        self.log(f"АнтИRaT: скан завершён, найдено меток: {len(suspicious_hits)}", "info")

    def isolate_antirat_target(self):
        target = prompt(self, "Изолировать", "Введите PID процесса или имя службы:")
        if not target:
            return
        if target.isdigit():
            pid = int(target)
            try:
                p = psutil.Process(pid)
                exe = Path(p.exe()) if p.exe() else None
                p.kill()
                self.log(f"Изоляция: процесс PID={pid} завершён", "malicious")
                if exe and exe.exists():
                    self.quarantine_file(exe)
            except Exception as e:
                self.log(f"Изоляция PID={pid} не удалась: {e}", "error")
            return

        service = target.strip()
        self.run_cmd_async(f"sc stop {service}")
        self.run_cmd_async(f"sc config {service} start= disabled")
        self.log(f"Изоляция службы выполнена: {service}", "malicious")
